...
namespace FieldOps.Modules.Reports.Application.Reports.Commands
{
    ...
    internal sealed class RemoveReportAssigneeCommandHandler(IReportsReadRepository repository, IOutboxMessagesRepository outboxRepository, IReportsUnitOfWork unitOfWork,
        IReportEventMapper eventMapper, IFilesModuleApi filesModuleApi) : IMessageHandler<RemoveReportAssigneeCommand>
    {
        public async Task HandleAsync(RemoveReportAssigneeCommand message, CancellationToken ct)
        {
            ...
            report.RemoveAssignee(message.FileId);
